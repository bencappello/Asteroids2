...
Game.prototype.reset = function () {
  ...
  this.asteroids = that.addInitialAsteroids();
  this.bullets = [];
  this.ship = new Asteroids.Ship({ pos: [this.width/2, this.height/2] });
};
...
Game.prototype.newLife = function () {
  ...
  lives -= 1;
  if (lives <= 0) {
    this.explosion.explode(function() {
      currentGame.gameOver = true;
    }, this.ship.pos);
  } else {
    this.explosion.explode(this.reset.bind(this), this.ship.pos);
  }
};
...
Game.prototype.allObjects = function () {
  var objects = this.bullets.concat(this.asteroids).push(this.explosion);
  if (this.ship) {
